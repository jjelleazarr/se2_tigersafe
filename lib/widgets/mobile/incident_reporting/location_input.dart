...
class _LocationInputState extends State<LocationInput> {
  ...
  Future<void> _selectOnMap() async {
    final selectedLocation = await Navigator.of(context).push<LatLng>(
      MaterialPageRoute(fullscreenDialog: true, builder: (ctx) => const MapScreen()),
    );

    if (selectedLocation != null) _selectPlace(selectedLocation.latitude, selectedLocation.longitude);
  }
  ...
  List<Widget> _buildLocationButtons() {
    return [
      TextButton.icon(
        icon: const Icon(Icons.location_on, color: Color(0xFFFEC00F)),
        label: const Text('Get Current Location', style: TextStyle(color: Color(0xFFFEC00F))),
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (ctx, constraints) {
            ...
            return isSmall
                ? Column(children: _buildLocationButtons())
                : Row(mainAxisAlignment: MainAxisAlignment.center, children: _buildLocationButtons());
          },
        ),
      ],
    );
