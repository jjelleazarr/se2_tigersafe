...
class _ImageInputState extends State<ImageInput> {
  ...
  Future<void> _recordVideo() async {
    ...
    setState(() => _selectedMedia.add(File(pickedVideo.path)));
    widget.onPickMedia(_selectedMedia);
  }
  ...
  Future<void> _pickMedia() async {
    ...
    if (result != null) {
      setState(() => _selectedMedia.addAll(result.paths.map((path) => File(path!))));
      widget.onPickMedia(_selectedMedia);
    }
  }
                        ...
                        child: isVideo
                            ? Column(
                                children: [
                                  ...
                                  Text(file.path.split('/').last, style: const TextStyle(fontSize: 12)),
                                ],
                              )
                            : ClipRRect(
                                borderRadius: BorderRadius.circular(10),
